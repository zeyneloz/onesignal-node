...
var Notification = function (initialBody) {
	...
	this.postBody = {}; 
	//added suport for subtitle,headings,ios_badCOunt, iosbadgeType
	if ('contents' in initialBody) {this.postBody.contents = initialBody.contents;}
	if ('subtitle' in initialBody) {this.postBody.subtitle = initialBody.subtitle;}
	if ('headings' in initialBody) {this.postBody.headings = initialBody.headings;}
	if ('ios_badgeCount' in initialBody) {this.postBody.ios_badgeCount = initialBody.ios_badgeCount;}
	if ('ios_badgeType' in initialBody) {this.postBody.ios_badgeType = initialBody.ios_badgeType;}
	if ('content_available' in initialBody){this.postBody.content_available = initialBody.content_available;}
	if ('template_id' in initialBody) {this.postBody.template_id = initialBody.template_id;}
